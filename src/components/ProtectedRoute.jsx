import { useAuth } from '../lib/AuthContext'

export default function ProtectedRoute({ children }) {
  const { user, loading, signInWithGoogle } = useAuth()

  if (loading) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: 'calc(100vh - 56px)', color: 'var(--text-3)' }}>
        Loading...
      </div>
    )
  }

  if (!user) {
    return (
      <div style={{
        position: 'fixed', inset: 0, background: 'rgba(30,34,53,0.35)',
        zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center',
      }}>
        <div style={{
          background: '#fff', borderRadius: 'var(--radius-lg)',
          padding: '36px 32px', maxWidth: '380px', width: '90%',
          textAlign: 'center', boxShadow: '0 8px 32px rgba(30,34,53,0.12)',
        }}>
          {/* <div style={{ fontSize: '2rem', marginBottom: '12px' }}>🔬</div> */}
          <div style={{ fontWeight: 600, fontSize: '1.1rem', marginBottom: '8px', color: 'var(--text-1)' }}>
            Sign in required
          </div>
          <div style={{ fontSize: '0.875rem', color: 'var(--text-2)', marginBottom: '24px', lineHeight: 1.6 }}>
            You need a UCR account to access this page. Please sign in with your UCR Google account to continue.
          </div>
          <button
            onClick={signInWithGoogle}
            style={{
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px',
              width: '100%', padding: '11px 20px', borderRadius: 'var(--radius-md)',
              background: 'var(--accent)', color: '#fff', border: 'none',
              fontSize: '0.95rem', fontWeight: 500, cursor: 'pointer',
              marginBottom: '10px',
            }}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none">
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#fff"/>
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#fff" fillOpacity=".7"/>
              <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" fill="#fff" fillOpacity=".5"/>
              <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#fff" fillOpacity=".3"/>
            </svg>
            Sign in with Google
          </button>
          <div style={{ fontSize: '0.78rem', color: 'var(--text-3)', marginBottom: '12px' }}>
            UCR accounts (@ucr.edu) only
          </div>
          <a href="/" style={{
            fontSize: '0.82rem', color: 'var(--brand)',
            textDecoration: 'underline', cursor: 'pointer',
          }}>
            ← Home Page
          </a>
        </div>
      </div>
    )
  }

  return children
}