...
for model_name, results_dict in model_lookup.items():
    for target, result in results_dict.items():
        model = result['best_estimator']
        filename = f"{save_dir}/{model_name}_{target}.joblib"
        joblib.dump(model, filename)

# Save feature column order (critical!)
joblib.dump(X.columns.tolist(), f"{save_dir}/feature_columns.joblib")
